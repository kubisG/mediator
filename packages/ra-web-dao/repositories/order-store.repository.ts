...
import { RaOrderStore } from "../../core/db/entity/ra-order-store";
import { EntityRepository } from "typeorm";
import { OrdStatus } from "../../core/enums/ord-status.enum";
import { RaAllocation } from "../../core/db/entity/ra-allocation";
import { TIF } from "../../core/enums/tif.enum";
import { OrderAllocated } from "../../core/enums/order-allocated.enum";

@EntityRepository(RaOrderStore)
export class OrderStoreRepository extends BaseRepository<RaOrderStore> {
    ...
    public async getSleuth(companyId: number, dateFrom: Date, side: string, symbol: string) {
        const result = await this.createQueryBuilder("ord")
            .where("ord.\"Symbol\"=:symbol and ord.\"Side\"=:side and ord.\"companyId\"=:comp"
                , { symbol: symbol, side: side, comp: companyId })
