import { Amqp } from "./providers/amqp";
import { Queue } from "./providers/queue.interface";
import { queueType } from "./constants";
import { LoggerService } from "@ra/web-core-be/logger/logger.service";
import { EnvironmentService } from "@ra/web-env-be/environment.service";
import { Options } from "amqplib";
import { Nats } from "./providers/nats";
...
export const queueFactory = [{
    ...
    useFactory: async (
        ...
        queue = new Nats(loggerService.logger, connect);

        await queue.connect();
        closeHandlers.push(async () => {
            loggerService.logger.warn(`Closing Nats`);
            // await queue.close();
        });
        return queue;
    },
