...
@Injectable()
export class TraderIoisService extends IoisService {
    ...
    public async getIois(token: string, dates: string, compOrders: boolean = false) {
        return super.getIois(token, dates, compOrders);
    }
    ...
    public async sendIois(data: any, token: string) {
        ...
        if (data.TargetCompID instanceof Array) {
            repeat = data.TargetCompID.length;
        }
        ...
        for (let i = 0; i < repeat; i++) {
            ...
            try {
                ...
                if (data.TargetCompID instanceof Array) {
                    treatedMessage["TargetCompID"] = data.TargetCompID[i];
                    sended["TargetCompID"] = data.TargetCompID[i];
                } else {
